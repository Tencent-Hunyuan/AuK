...
class AukInfer:
    def __init__(
        ...
        *,
        ...
        # Optional CPU offload: the Qwen encoder and the DiT stay on the host when idle and are
        # pulled onto their own device by accelerate hooks. The VAE is never offloaded.
        ...
        logger.info(
            "Devices | dit=%s | qwen=%s | vae=%s | autocast=%s | weights=%s | cpu_offload=%s",
            self.dit_device,
            self.qwen_device,
            self.vae_device,
            dtype,
        ...
        # Keep the VAE resident on its own device. Only Qwen and DiT are ever offloaded, which
        # preserves the baseline VAE weight_norm execution path and avoids VAE CPU<->GPU transfers.
        # Without --device_map this is the DiT device, i.e. the historical behaviour.
        ...
        if cpu_offload:
            ...
            for name, _ in self.model.named_parameters(recurse=False):
                set_module_tensor_to_device(self.model, name, self.fusion_device)
            # A split-deployment worker carries no LLM of its own, so only hook what is loaded.
            _, text_hook = (
                cpu_offload_with_hook(self.model.text_encoder, self.qwen_device)
                if self.model.text_encoder is not None
                else (None, None)
            )
            _, transformer_hook = cpu_offload_with_hook(self.model.transformer, self.dit_device, prev_module_hook=text_hook)
    ...
    def _place_submodules(self, model: CFMEdit) -> None:
        ...
        # remembered so the CPU-offload hooks can keep these small tensors on the right card
        self.fusion_device = fusion_device
